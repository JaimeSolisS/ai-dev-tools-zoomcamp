...
def list_guest_links(session_id: str, user: CurrentUser, store: StoreDep):
    return store.list_links(user, session_id)


@router.post("", response_model=CreatedGuestLink, status_code=201)
def create_guest_link(session_id: str, user: CurrentUser, store: StoreDep, body: CreateGuestLinkInput | None = None):
    link, token = store.create_link(user, session_id, body or CreateGuestLinkInput())
    return CreatedGuestLink(link=link, token=token)


@router.delete("/{link_id}", status_code=204)
def revoke_guest_link(session_id: str, link_id: str, user: CurrentUser, store: StoreDep):
