...
Companion to [plan.md](plan.md). Describes _how_ the MVP is built. No code yet.
...
- **`gpt-4o-transcribe-diarize` gives speaker labels.** Knowing who said what is
  what makes action-item _owner_ extraction reliable — without it the model
...
- **Participation metrics survive via `CycleParticipation`.** It records _that_
  a member submitted and _how many_ cards, never which ones. The summary screen's
...
- the project roster in; decisions, action items with owner _names_, due dates,
...
1. **Can members edit cards after reveal?** The plan implies no ("_before_ the
